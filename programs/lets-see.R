...
CPS[, SES := rowMeans(cbind(
  # z_DadEduc, z_MomEduc, 
  z_mom_earnweek, z_pop_earnweek), 
                      na.rm = TRUE)]
...
lw_index <- function(data, 
  ...
  # Track original row indices from the input data
  DT[, row_index := as.integer(attr(DT, "row.names"))]

  # Return results, including matched row index
  results <- list(
...
# Safely assign LW index using row indices returned from lw_index
CPS[, ses_lw := NA_real_]

if (length(results$index) != length(results$row_index)) {
  stop("Mismatch: results$index and results$row_index have different lengths.")
}

CPS[results$row_index, ses_lw := results$index]

# Now, the CPS dataset has the index added to it
# You can check the summary of the new index variable
# summary(CPS$ses_lw)

# Plot distribution of LW Index
if (sum(!is.na(CPS$ses_lw)) > 1) {
  hist(CPS$ses_lw, main = "Distribution of LW Index", xlab = "LW Index")
  
  # Standardize and plot
  CPS[, ses_lw_std := scale(ses_lw)]
  hist(CPS$ses_lw_std, main = "Distribution of Standardized LW Index", xlab = "Z-scored LW Index")
} else {
  CPS[, ses_lw_std := NA_real_]
  warning("Not enough non-NA values in ses_lw to plot or standardize.")
}
